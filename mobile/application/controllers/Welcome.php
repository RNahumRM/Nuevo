...
//                redirect($_SERVER['HTTP_REFERER']);
    ...
    public function indexAjaxDataUnits(){
        ...
        */
        ...
        $waiting = $this->alarms->all('waiting', $tenants);
        $attending = $this->alarms->all('attending', $tenants);
    ...
    public function indexAjaxDataAlarms(){
        ...
        $alarms = $this->alarms->all($filter, $tenants);
    ...
    public function users()
    {
        try {
            ...
            if ($this->session->userdata("idUser")) {
                ...
                if( mb_strtoupper($this->session->userdata('profile')) == 'READ' ) {
                    $crud->where('email', $this->session->userdata('email'));
                    $crud->fields('image', 'email', 'password', 'name');
                    $crud->unset_add();
                    $crud->unset_delete();
                } else {
...
//                $crud->unset_jquery_ui();
    ...
    public function updateCamerasFromCameraManager(){
        ...
        if($cameraIds) {
            ...
            if ($token) {
                ...
                foreach ($cameraIds as $cameraId) {
                    ...
                    if(array_key_exists('code', $streams)){
                        $success = 0;
                        $data['detail'] = $streams['detail'];
                    } else {
    ...
    public function recordings(){
        if( $this->session->userdata('idUser') ) {
            ...
            $data["recordings"] = $this->units->cameras($idUnit);
            $data["view"] = "recordings";

            $data["title"] = 'Grabaciones';
            print_r($data);
            $this->load->view("template", $data);
